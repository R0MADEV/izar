...
function buildZodSchema(toolParameters: Tool['parameters']): z.ZodObject<Record<string, z.ZodTypeAny>> {
  const zodShape: Record<string, z.ZodTypeAny> = {}

  for (const [paramName, paramDefinition] of Object.entries(toolParameters)) {
    const baseType =
      paramDefinition.type === 'string' ? z.string() :
      paramDefinition.type === 'number' ? z.number() :
      z.boolean()

    const annotatedType = baseType.describe(paramDefinition.description)
    zodShape[paramName] = paramDefinition.required === false
